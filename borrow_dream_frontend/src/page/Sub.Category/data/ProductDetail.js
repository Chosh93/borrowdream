...
import ProductOrder from '../../payment/productOrder';
...
const ProductDetail = ( { cart, setCart, convertPrice }) => {
  ...
  const handleClick = () => {
    console.log(product.pname);
    const encodedProductName = encodeURIComponent(product.pname);
    navigate(`/productOrder?id=${encodedProductName}`, { state: {productName: product.pname } });
  };
      ...
      <Main>
        <ProductContainer>
          ...
          <ContentContainer>
            ...
            <ButtonContainer>
              <Button className='buy' onClick={handleClick}>구매하기</Button>
